...
stackChecler_cmd = (
    ...
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 Stack - أوامـر الصيد والتشيكـر](t.me/Stack71) 𓆪\n\n"
    ...
    "**- انضـم للقنـاة ~ $stack71@@**\n"

    "**⪼ لـ رؤيـة بعـض اليـوزرات التي قام بصيدهـا منصبيـن stack71**\n\n"
...
"𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 Stack - أنـواع اليـوزرات](t.me/stack71) 𓆪\n\n"
...
"**⪼ لـ رؤيـة بعـض اليـوزرات التي قام بصيدهـا منصبيـن stack**"
...
@stack.zed_cmd(pattern="الصيد")

async def cmd(stacklll):

    await edit_or_reply(stacklll, stackChecler_cmd)



@stack.zed_cmd(pattern="النوع")

async def cmd(stacklll):

    await edit_or_reply(stacklll, stackType_cmd)



@stack.zed_cmd(pattern="صيد (.*)")
...
async def hunterusername(event):
    ...
    if choice not in ("ثلاثيات", "خماسيات", "خماسي حرفين", "سداسيات", "سداسي حرفين", "سباعي", "بوتات"): #code by t.me/stack71
    ...
    try:
        ...
        if replly and replly.text.startswith('@'): #Code Update by @stack71
            ...
            ch = await stack(
                ...
                functions.channels.CreateChannelRequest(
                    ...
                    title="⎉ صيـد stack Stack ⎉",

                    about="This channel to hunt username by - @UA9U9 ",
        ...
        await stack.send_message(event.chat_id, f"خطأ في انشاء القناة , الخطأ**-  : {str(e)}**")
    ...
    while zedmod: #code by t.me/stack71
        ...
        if isav == True:
            ...
            except telethon.errors.FloodWaitError as e: #code by t.me/stack71
...
@stack.zed_cmd(pattern="تثبيت (.*)")

async def _(event):

    stack = str(event.pattern_match.group(1))

    if not stack.startswith('@'): # Code Update by @stack71
    ...
    try:
        ...
        ch = await stack(
            ...
            functions.channels.CreateChannelRequest(
                ...
                title="⎉ تثبيت ستاك Stack ⎉",

                about="تم تثبيت اليـوزر بواسطـة سـورس  ستاك - @UA9U9 ",
    ...
    username = stack.replace("@", "")  # Code Update by @stack71
    ...
    return await stack.send_message(event.chat_id, "**- تم الانتهاء من التثبيت .. بنجـاح ✅**")





@stack.zed_cmd(pattern="ثبت (.*)")

async def _(event): # Code Update by @stack71
    ...
    while swapmod:
        ...
        if isav == True:
            ...
            try: # Code Update by @stack71

                await stack(functions.account.UpdateUsernameRequest(username=username))
...
@stack.zed_cmd(pattern="حالة الصيد")
...
@stack.zed_cmd(pattern="حالة التثبيت")
...
@stack.zed_cmd(pattern="ايقاف الصيد")

async def _(event): #code by t.me/stack71
...
@stack.zed_cmd(pattern="ايقاف التثبيت")
